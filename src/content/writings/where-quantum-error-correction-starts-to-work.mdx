...
Everything runs on the field's standard open tools. [Stim](https://github.com/quantumlib/Stim) builds and simulates the faulty circuits. [sinter](https://pypi.org/project/sinter/) runs thousands of them in parallel and collects the statistics. [PyMatching](https://github.com/oscarhiggott/PyMatching) is the pairing decoder from the last section. Exact versions are pinned in the repository.

We also ran the same circuit and noise settings with [Fusion Blossom](https://github.com/yale-paragon/fusion-blossom), an independent implementation of the same pairing algorithm. Each decoder received separately sampled shots. The two produce similar failure rates across the sweep, giving us a consistency check on the results.
